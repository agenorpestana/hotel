...
$query = $pdo->query("SELECT * from $pagina where referencia = 'Serviço' and pago = 'Não'");
...
if($alterou_data == 'Sim'){
	...
	if($dataInicial != "" || $dataFinal != ""){
		...
		$query = $pdo->query("SELECT * from $pagina where (data_lanc >= '$dataInicial' and data_lanc <= '$dataFinal') and pago LIKE '$status' and referencia = 'Serviço' order by id desc ");

	}

}else if($status != '%%' and $alterou_data == ''){

	$query = $pdo->query("SELECT * from $pagina where pago LIKE '$status' and referencia = 'Serviço' order by id desc ");
	...
	$query = $pdo->query("SELECT * from $pagina where data_venc < curDate() and pago = 'Não' and referencia = 'Serviço' order by id desc ");
	...
	$query = $pdo->query("SELECT * from $pagina where data_venc = '$data_amanha' and pago = 'Não' and referencia = 'Serviço' order by id desc ");
	...
	$query = $pdo->query("SELECT * from $pagina WHERE data_lanc = curDate() and referencia = 'Serviço' order by id desc ");
...
echo <<<HTML
		...
		<p>Confirmar Baixa? <a href="#" onclick="baixar_conta('{$id}')"><span class="text-danger">Sim</span></a></p>
					...
					<big><a class="" href="rel/comprovante.php?id={$id}&imprimir=Não" target="_blank" title="Ver Comprovante"><i class="fa fa-file-pdf-o text-danger"></i></a></big>
...
<script type="text/javascript">
	...
	function editar(id, descricao, valor, data_venc, arquivo, obs, hospede){
