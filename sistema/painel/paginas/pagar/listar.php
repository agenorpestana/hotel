...
echo <<<HTML
			...
			<tr> 
				...
				<i class="fa fa-square {$classe_pago} mr-1"></i> {$descricao} <span class="{$classe_pessoa}"><small>($nome_pessoa)</small></span></td> 
				...
				<td class="esc">{$forma_pgtoF}</td>

				<td><a href="images/contas/{$arquivo}" target="_blank"><img src="images/contas/{$tumb_arquivo}" width="30px" height="30px"></a></td>

				<td>

					<big><a class="{$ocultar}" href="#" onclick="editar('{$id}', '{$descricao}', '{$fornecedor}','{$valor}','{$data_venc}','{$arquivo}', '{$funcionario}', '{$obs}','{$hospede}','{$forma_pgto}')" title="Editar Dados"><i class="fa fa-edit text-primary "></i></a></big>
		...
		<p>Confirmar Baixa?<br> <b>Forma de PGTO: </b>{$forma_pgtoF} <a href="#" onclick="baixar_conta('{$id}')"><span class="text-danger"> Sim</span></a></p>
...
<script type="text/javascript">
	...
	function editar(id, descricao, fornecedor, valor, data_venc, arquivo, funcionario, obs, hospede, forma_pgto){
