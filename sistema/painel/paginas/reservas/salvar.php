...
$query = $pdo->prepare("INSERT INTO $tabela SET hospede = :hospede, tipo_quarto = :tipo_quarto, quarto = :quarto, funcionario = :funcionario, check_in = :check_in, check_out = :check_out, valor = :valor, no_show = :no_show, hospedes = :hospedes, obs = :obs, valor_diaria = :valor_diaria, data = curDate(), desconto = :desconto, forma_pgto = :forma_pgto, hospedes_criancas = :hospedes_criancas, indicacao = :indicacao");
...
$query = $pdo->prepare("UPDATE $tabela SET hospede = :hospede, tipo_quarto = :tipo_quarto, quarto = :quarto, funcionario = :funcionario, check_in = :check_in, check_out = :check_out, valor = :valor, no_show = :no_show, hospedes = :hospedes, obs = :obs, valor_diaria = :valor_diaria, desconto = :desconto, forma_pgto = :forma_pgto, hospedes_criancas = :hospedes_criancas, indicacao = :indicacao where id = '$id'");
...
$query->bindValue(":hospedes_criancas", "$hospedes_criancas");
...
if($id == ""){
	...
	$ultimo_id = $id;
	if($no_show > 0){
		//verificar se já tem noshow lançado anteriormente
		$query2 = $pdo->query("SELECT * from receber where referencia = 'Entrada' and id_ref = '$id'");
		$res2 = $query2->fetchAll(PDO::FETCH_ASSOC);
		if(@count($res2) == 0){
			$pdo->query("INSERT INTO receber SET descricao = 'Entrada Reserva', valor = '$no_show', data_venc = curDate(), data_lanc = curDate(), usuario_lanc = '$id_usuario', arquivo = 'sem-foto.png', pago = 'Sim', data_pgto = curDate(), usuario_pgto = '$id_usuario', hospede = '$hospede', referencia = 'Entrada', id_ref = '$ultimo_id', forma_pgto = '$forma_pgto', hora = curTime(), caixa = '$id_caixa'");	
		}else{
			
			$pdo->query("UPDATE receber SET valor = '$no_show', data_pgto = curDate(), usuario_pgto = '$id_usuario', hospede = '$hospede', forma_pgto = '$forma_pgto', hora = curTime(), caixa = '$id_caixa' where referencia = 'Entrada' and id_ref = '$id'");	
		}
	}
