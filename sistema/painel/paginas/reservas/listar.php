...
for($i=0; $i<$linhas; $i++){
	...
	$reserva_site = $res[$i]['reserva_site'];
	$hospedes_criancas = $res[$i]['hospedes_criancas'];
	$idade_1 = $res[$i]['idade_1'];
	$idade_2 = $res[$i]['idade_2'];
$idade_3 = $res[$i]['idade_3'];
$idade_4 = $res[$i]['idade_4'];
$idade_5 = $res[$i]['idade_5'];
$idade_6 = $res[$i]['idade_6'];
$idade_7 = $res[$i]['idade_7'];
$idade_8 = $res[$i]['idade_8'];

$indicacao = $res[$i]['indicacao'];
$descricao_taxa = $res[$i]['descricao_taxa'];
$valor_taxa = $res[$i]['valor_taxa'];
$valor_taxaF = @@number_format($valor_taxa, 2, ',', '.');  

	$reserva_siteF = '';
	if($reserva_site != ""){
		$reserva_siteF = '<span style="color:red; font-size:12px">(Site)</span>';
	}
	...
	$nome_hospedeF = mb_strimwidth($nome_hospede, 0, 15, "...");
...
<div class="col-md-4 col-12 widget largura_do_card" style="margin-right: 5px; margin-bottom: 5px;">
		...
		<span class="{$classe_obs}"><br><b>OBS:</b> {$obs}</span>
				...
				<div class="stats">
					...
					<h5 style="font-size:14px"><strong>{$numero_quarto} ({$nome_tipoF}) 

					<a title="Editar Reserva" href="#" onclick="editar('{$id}', '{$hospede}', '{$check_in}', '{$check_out}', '{$tipo_quarto}', '{$quarto}', '{$valor_diaria}', '{$valor}', '{$no_show}', '{$hospedes}', '{$obs}', '{$desconto}', '{$forma_pgto}', '{$idade_1}', '{$idade_2}', '{$idade_3}', '{$idade_4}', '{$idade_5}', '{$idade_6}', '{$idade_7}', '{$idade_8}', '{$hospedes_criancas}', '{$indicacao}'
)"><img style="margin-left: 10px" src="images/edit.png" width="15px"></a>

					</strong></h5>

					<span style="font-size:13px">{$nome_hospedeF} {$reserva_siteF}</span>
...
<div style="margin-top: 10px; position:fixed; bottom:10px; right:10px; padding-left: 5px;">
...
<script type="text/javascript">
	...
	let criandoReserva = false;


	function editar(id, hospede, check_in, check_out, tipo_quarto, quarto, valor_diaria, valor, noshow, hospedes, obs, desconto, forma_pgto, idade_1, idade_2, idade_3, idade_4, idade_5, idade_6, idade_7, idade_8, hospedes_criancas, indicacao){
...
setTimeout(function() {
    // Passa o valor da quantidade de crianças para o campo 'hospedes_criancas'
    $('#hospedes_criancas').val(hospedes_criancas);

    // Dispara o evento de 'input' manualmente para criar os inputs de idades
    $('#hospedes_criancas').trigger('input'); 

    criarCamposIdade();

    // Preenche os valores das idades das crianças, após a criação dos inputs
    
}, 1000);


setTimeout(function() {
    $('#idade_crianca_1').val(idade_1);
    $('#idade_crianca_2').val(idade_2);
    $('#idade_crianca_3').val(idade_3);
    $('#idade_crianca_4').val(idade_4);
    $('#idade_crianca_5').val(idade_5);
    $('#idade_crianca_6').val(idade_6);
    $('#idade_crianca_7').val(idade_7);
    $('#idade_crianca_8').val(idade_8);
    calcular();
...
setTimeout(function() {    
    calcular();
		...
		$('#tipo_quarto').on('change', function () {

			if (editandoReserva) {
				// NÃO recalcula diária ao editar
				return;
			}

			// aqui entra o cálculo normal da diária padrão
			// $('#valor_diaria').val(valor_padrao);
		});
	...
	function fazer_checkout(id, hospede, quarto, valor, obs, hospedes,  hora, telefone, funcionario, tipo_pgto, data, hora_checkin, numero, restante, restanteF, descricao_taxa, valor_taxa){
		...
		$('#modalCheckout').modal('show');
	    	...
	    	$('#taxa_extra_dados_checkout').text(valor_taxa);
	    	$('#descricao_taxa_dados_checkout').text(descricao_taxa);
