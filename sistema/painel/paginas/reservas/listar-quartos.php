...
if($linhas > 0){
	...
	for($i=0; $i<$linhas; $i++){
		...
		if(@count($res2) == 0 || ($id_quarto == $id_quarto_post and strtotime($check_in) >= strtotime($check_in_reserva) and $check_out <= $check_out_reserva)){
			...
			$query2 = $pdo->query("SELECT * from reservas where (quarto = '$id_quarto' and check_in < '$check_out' and check_out > '$check_out' and id != '$id') or (quarto = '$id_quarto' and check_in > '$check_in' and check_out <= '$check_out' and id != '$id') order by id desc");
