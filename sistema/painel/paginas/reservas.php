							...
							<div class="row">					
								...
								<div class="col-md-3">							
									...
									<div class="row" id="linha-criancas">
									    <div class="col-md-3">
									        <label>Hóspedes Crianças</label>
									        <input type="number" class="form-control" id="hospedes_criancas" name="hospedes_criancas" min="0" value="0" required onchange="criarCamposIdade()">
									    </div>
									    <!-- Inputs dinâmicos virão aqui -->
									...
									<div class="row">
										...
										<div class="col-md-4">
									        ...
									        <input type="text" class="form-control" id="indicacao" name="indicacao"  placeholder="Nome da Pessoa">
									    </div>
		...
		<span></span>
								...
								<div class="row">
									...
									<div class="col-md-6">
										...
										<button title="Novo Hóspede" onclick="novoHospede()" class="btn btn-primary"><i class="fa fa-plus "></i></button>
										</div>	

											<div class="col-md-10">	
												<div id="div_hospedes_reserva">
											...
											<div class="col-md-1" style="padding:0">		
												<button onclick="addHospede()" class="btn btn-success"><i class="fa fa-check "></i></button>
											</div>									
								...
								<div class="row">
									<div class="col-md-2">						
										...
										<div class="col-md-7">						
											...
											<input type="text" class="form-control" id="descricao_taxa" name="descricao_taxa" placeholder="Ex: Diária Adicional" >						
										</div>


										<div class="col-md-3">						
											<label>Valor Taxa Extra</label>
											<input type="text" class="form-control" id="valor_taxa" name="valor_taxa" placeholder="" >						
										</div>
								...
								<table style="border-bottom-style: solid; font-size: 15px; margin-bottom:10px; width: 100%; table-layout: fixed;">
									...
									<thead> 
										...
										<tr style="margin-left: 0px; background-color:#f5f5f5; font-weight: bold"> 
											...
											<th style="width:40%">Hóspede Principal</th>	

											<th style="width:30%"><span id="descricao_taxa_dados_checkout"></span></th>	
									...
									<tbody>
										...
										<tr style="margin-left: 0px;"> 
											...
											<td style="width:60%" id="taxa_extra_dados_checkout"></td>	
...
<div class="modal fade" id="modalHospedes" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
	...
	<div class="modal-dialog">
		...
		<div class="modal-content">
			...
			<div class="modal-header">
				...
				<h4 class="modal-title" id="exampleModalLabel">Novo Hóspede</h4>

				<button id="btn-fechar-hospedes" type="button" class="close" data-dismiss="modal" aria-label="Close" style="margin-top: -25px">
	...
	<div class="col-md-9">							
				...
				<small><div id="mensagem_hospedes" align="center"></div></small>
...
<input type="hidden" id="idade_capturada">
        	...
        	calcular();       
					...
					function calcular(checkin, checkout, diaria){
						...
						var idade_crianca_1 = $('#idade_crianca_1').val();
						var idade_crianca_2 = $('#idade_crianca_2').val();
						var idade_crianca_3 = $('#idade_crianca_3').val();
						var idade_crianca_4 = $('#idade_crianca_4').val();
						var idade_crianca_5 = $('#idade_crianca_5').val();
						var idade_crianca_6 = $('#idade_crianca_6').val();
						var idade_crianca_7 = $('#idade_crianca_7').val();
						var idade_crianca_8 = $('#idade_crianca_8').val();
						...
						$.ajax({
							...
							data: {checkin, checkout, diaria, desconto, idade_crianca_1, idade_crianca_2, idade_crianca_3, idade_crianca_4, idade_crianca_5, idade_crianca_6, idade_crianca_7, idade_crianca_8 },
...
$("#form_hospedes").submit(function () {
    ...
    $.ajax({
        url: 'paginas/hospedes/salvar.php',
        type: 'POST',
        data: formData,

        success: function (mensagem) {
            $('#mensagem_hospedes').text('');
            $('#mensagem_hospedes').removeClass()
            if (mensagem.trim() == "Salvo com Sucesso") {

                $('#btn-fechar-hospedes').click();
                listar();   
                listarSelectHospedes('Sim');          

            } else {

                $('#mensagem_hospedes').addClass('text-danger')
                $('#mensagem_hospedes').text(mensagem)
            }
...
function addHospede(){
	var id_reserva = $('#id_checkin').val();
	var hospede = $('#hospedes_reserva').val();
	if(hospede == ""){
		alert('Selecione um Hóspede!');
		return;
	}
		$.ajax({
			url: 'paginas/' + pag + "/associar_hospede.php",
			method: 'POST',
			data: {id_reserva, hospede},
			dataType: "html",
			success:function(result){				       
				listar_hosp(id_reserva);
			}
		});
...
function listarSelectHospedes(valor){	
	
		$.ajax({
			url: 'paginas/' + pag + "/listar_select_hospedes.php",
			method: 'POST',
			data: {valor},
			dataType: "html",
			success:function(result){				       
				$("#div_hospedes_reserva").html(result);     
			}
		});
...
function listarDetalhamento(){	

	var id_reserva = $('#id_checkout').val();
	
		$.ajax({
			url: 'paginas/' + pag + "/listar_detalhamento.php",
			method: 'POST',
			data: {id_reserva},
			dataType: "html",
			success:function(result){				       
				$("#listar_detalhamento").html(result);     
			}
		});
...
function criarCamposIdade() {
    ...
    // Verifica se a quantidade é maior que 8
    if (quantidade > 8) {
        alert("O número de crianças não pode ser superior a 8.");
        $('#hospedes_criancas').val(8); // Limita o valor para 8
        quantidade = 8; // Atualiza a variável para 8
    }

    const linha = document.getElementById('linha-criancas');

    // Salva os valores atuais antes de remover os campos
    ...
    // Remove colunas antigas (exceto a primeira)
    while (linha.children.length > 1) {
    ...
    for (let i = 1; i <= quantidade; i++) {
        ...
        // Reatribui o valor salvo, se existir
        if (valoresExistentes[input.name]) {
            input.value = valoresExistentes[input.name];
        }

        // Evento para chamar a função de cálculo
